...
export class VeterinarioService {
  ...
  async updateVeterinario(
    ...
    const veterinarioActualizado = await this.veterinarioModel.findByIdAndUpdate(
      id,
  ...
  async deleteMultipleVeterinarios(ids: string[]): Promise<Veterinario[]> {
    const veterinariosAEliminar = await this.veterinarioModel.find({ _id: { $in: ids } }).exec();

    if (veterinariosAEliminar.length === 0) {
      throw new NotFoundException('No veterinarios found to delete.');
    }

    const result = await this.veterinarioModel.deleteMany({ _id: { $in: ids } }).exec();
