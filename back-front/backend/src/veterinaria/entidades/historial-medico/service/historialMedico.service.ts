...
export class HistorialMedicoService {
  ...
  async agregarEntradaAHistorial(mascotaId: string, entrada: any) {
    ...
    if (!tieneCitas) {
      throw new NotFoundException(
        `La mascota con ID ${mascotaId} no tiene citas registradas. No se puede agregar una entrada al historial médico.`,
      );
    }
    ...
    if (!historial) {
      throw new NotFoundException(
        `No se encontró historial médico para la mascota con ID ${mascotaId}`,
      );
    }
  ...
  async findHistorialByDescripcion(texto: string): Promise<HistorialMedico[]> {
    const mascotasCoincidentes = await this.mascotaModel
      .find({
  ...
  async findAllHistorialesMedicos(): Promise<any[]> {
    return this.historialMedicoModel.find({}).populate('mascotaID', 'nombre').exec();
  }
  ...
  async updateHistorialMedico(
    ...
    const historialActualizado = await this.historialMedicoModel.findByIdAndUpdate(
      id,
  ...
  async deleteMultipleHistorialMedico(ids: string[]): Promise<HistorialMedico[]> {
    const historialesAEliminar = await this.historialMedicoModel.find({ _id: { $in: ids } }).exec();

    if (historialesAEliminar.length === 0) {
      throw new NotFoundException('No historial médicos found to delete.');
    }

    const result = await this.historialMedicoModel.deleteMany({ _id: { $in: ids } }).exec();
  ...
  async findHistorialPorMascota(mascotaId: string): Promise<HistorialMedico> {
    ...
    if (!historial) {
      throw new NotFoundException(
        `Historial médico no encontrado para la mascota con ID ${mascotaId}`,
      );
    }
   ...
   */
  async deleteEntrada(historialId: string, entradaId: string): Promise<HistorialMedico> {
    const historial = await this.historialMedicoModel.findById(historialId);
    if (!historial) {
      throw new NotFoundException(`Historial ${historialId} no encontrado`);
    }
