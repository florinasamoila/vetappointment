...
export class ServicioPrestadoController {
  ...
  async findServiciosPrestados(@Query('search') search?: string): Promise<ServicioPrestado[]> {
  ...
  async findServicioPrestadoById(@Param('id') id: string): Promise<ServicioPrestado> {
  ...
  async deleteMultipleServiciosPrestados(@Body() ids: string[]): Promise<void> {
    if (!ids || ids.length === 0) {
